...
export default function CheckoutScreen() {
  ...
  const L = {
    ...
    delivery:    language === "ar" ? "بيانات التسليم والتواصل" : language === "tr" ? "Teslimat ve Iletisim Bilgileri" : "Delivery & Contact Info",
    fullName:    language === "ar" ? "الاسم الكامل / الشركة" : language === "tr" ? "Tam Adi / Sirket" : "Full Name / Company",
    phone:       language === "ar" ? "رقم الهاتف" : language === "tr" ? "Telefon Numarasi" : "Phone Number",
    country:     language === "ar" ? "البلد" : language === "tr" ? "Ulke" : "Country",
    city:        language === "ar" ? "المدينة" : language === "tr" ? "Sehir" : "City",
    district:    language === "ar" ? "الحي / المنطقة" : language === "tr" ? "Ilce" : "District",
    fullAddress: language === "ar" ? "العنوان التفصيلي" : language === "tr" ? "Acik Adres" : "Full Address",
    ...
    notesLabel:  language === "ar" ? "ملاحظات للمطبعة (اختياري)" : language === "tr" ? "Matbaa Icin Ek Notlar" : "Notes for Printing (Optional)",
    notesPlc:    language === "ar" ? "ألوان الطباعة أو مدد التوصيل" : language === "tr" ? "Baski renkleri veya teslimat notlari" : "Print colors or delivery notes",
    payMethod:   language === "ar" ? "طريقة الدفع" : language === "tr" ? "Odeme Yontemi" : "Payment Method",
    cash:        language === "ar" ? "الدفع النقدي عند الاستلام" : language === "tr" ? "Nakit Odeme (Kapida)" : "Cash on Delivery",
    cashDesc:    language === "ar" ? "يتم تحصيل المبلغ عند تسليم المطبوعات" : language === "tr" ? "Teslim alindiginda tahsil edilecektir" : "Payment collected upon delivery",
    bank:        language === "ar" ? "تحويل بنكي" : language === "tr" ? "Banka Havalesi" : "Bank Transfer",
    bankDesc:    language === "ar" ? "تتم المعالجة بعد تأكيد وصول الحوالة" : language === "tr" ? "Havale onayindan sonra islem baslar" : "Processing starts after transfer confirmation",
    finalBill:   language === "ar" ? "الفاتورة النهائية" : language === "tr" ? "Son Fatura" : "Final Bill",
    products:    language === "ar" ? "المنتجات" : language === "tr" ? "Urunler" : "Products",
    tax:         language === "ar" ? "الضريبة" : language === "tr" ? "Vergi" : "Tax",
    grandTotal:  language === "ar" ? "الإجمالي الكلي" : language === "tr" ? "Genel Toplam" : "Grand Total",
    confirm:     language === "ar" ? "تأكيد الطلب الآن" : language === "tr" ? "Siparisi Onayla" : "Confirm Order Now",
    secure:      language === "ar" ? "عملية الطلب آمنة تماماً" : language === "tr" ? "Siparis islemi tamamen guvenlidir" : "Order process is completely secure",
  };

  const buildMessage = () => {
    const paymentStr = payment === "cash"
      ? (language === "ar" ? "نقدي عند الاستلام" : language === "tr" ? "Nakit Odeme" : "Cash on Delivery")
      : (language === "ar" ? "تحويل بنكي" : language === "tr" ? "Banka Havalesi" : "Bank Transfer");

    const header = language === "ar" ? "طلب جديد - Skyline Group\n" : language === "tr" ? "Yeni Siparis - Skyline Group\n" : "New Order - Skyline Group\n";

    const productsList = items.map((item) => {
      const cat = CATEGORIES.find((c) => c.id === item.product.categoryId);
      return `  - ${getProductName(item.product, language)} (${item.quantity}x)${cat ? " - " + getCategoryName(cat, language) : ""}`;
    }).join("\n");

    let msg = `${header}\n`;
    msg += `Name: ${customerName}\n`;
    msg += `Phone: ${customerPhone}\n`;
    msg += `City: ${city}${district ? " / " + district : ""}\n`;
    if (address.trim()) msg += `Address: ${address}\n`;
    msg += `\nProducts:\n${productsList}\n`;
    msg += `\nTotal: ${fmt(total)} TL\n`;
    msg += `Payment: ${paymentStr}\n`;
    if (notes.trim()) msg += `\nNotes: ${notes}`;
    return msg;
  };
  ...
  const handleConfirm = async () => {
    if (!customerName.trim() || !customerPhone.trim() || !city.trim()) {
      showAlert(t("fillAllFields")); return;
    }
    setSending(true);
    try {
      ...
      await saveOrder();
      clearCart();
      showAlert(t("orderSentDesc"), t("orderSent"));
      router.replace("/(tabs)");
    } catch {
      showAlert("Could not open WhatsApp", "Error");
    } finally {
      setSending(false);
    }
  };
      ...
      >
        ...
        <View style={s.formCard}>
          ...
          <View style={[s.row2, isRTL && s.rowR]}>
            <View style={s.half}>
              ...
              <TextInput style={inputStyle} placeholder="Turkiye" placeholderTextColor="#CCC" defaultValue="Turkiye" />
            </View>
            <View style={s.half}>
              <FieldLabel label={L.city} required isRTL={isRTL} />
              <TextInput style={inputStyle} placeholder="Gaziantep" placeholderTextColor="#CCC" value={city} onChangeText={setCity} />
            </View>
          </View>

          <FieldLabel label={L.district} isRTL={isRTL} />
          <TextInput style={inputStyle} placeholder="Sahinbey" placeholderTextColor="#CCC" value={district} onChangeText={setDistrict} />
        ...
        <View style={s.formCard}>
          ...
          <PayOption
            ...
            icon="B" title={L.bank} desc={L.bankDesc} isRTL={isRTL}
          />
        </View>
        ...
        <View style={s.billCard}>
          ...
          <View style={s.billBody}>
            <BRow label={L.products} value={`${fmt(subtotal)} TL`} isRTL={isRTL} />
            <BRow label={L.tax} value={`+ ${fmt(vat)} TL`} isRTL={isRTL} />
            <View style={s.divider} />
            <View style={[s.grandRow, isRTL && s.rowR]}>
              <Text style={s.grandLabel}>{L.grandTotal}</Text>
              <Text style={s.grandVal}>{fmt(total)} TL</Text>
            </View>
          </View>
